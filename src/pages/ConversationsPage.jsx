...
const ConversationsPage = () => {
  ...
  const characters = conversations
    ? [...new Set(conversations.map(conv => conv.characters?.name))]
        .filter(Boolean)
        .sort()
    : [];
  ...
  const filteredConversations = conversations
    ? conversations.filter(conv => {
        ...
        const matchesSearch = !filters.search || 
          ...
          conv.characters?.name.toLowerCase().includes(filters.search.toLowerCase());
        
        // Character filter
        const matchesCharacter = filters.character === 'all' || 
          conv.characters?.name === filters.character;
        
        // Favorites filter
        const matchesFavorite = !filters.favoritesOnly || conv.is_favorite;
        
        // Testament filter (if character has testament property)
        const matchesTestament = filters.testament === 'all' || 
          conv.characters?.testament === filters.testament;
        
        return matchesSearch && matchesCharacter && matchesFavorite && matchesTestament;
      })
    : [];
    ...
    <div className="min-h-screen bg-blue-900 text-white">
      ...
      <div className="container mx-auto px-4 py-8">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-8">
          <h1 className="text-3xl font-bold text-yellow-300 mb-4 md:mb-0">Your Conversations</h1>
          <Link 
            to="/" 
            className="px-4 py-2 bg-[rgba(250,204,21,.15)] border border-yellow-400 text-yellow-400 rounded-lg font-semibold hover:bg-yellow-400 hover:text-blue-900 transition-colors"
          >
        ...
        {/* Filters */}
        <div className="bg-[rgba(255,255,255,.05)] rounded-lg p-4 mb-8">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            <div>
              ...
              <input
                ...
                className="w-full px-3 py-2 bg-[rgba(255,255,255,.1)] border border-[rgba(255,255,255,.2)] rounded-md text-white placeholder-blue-200 focus:outline-none focus:ring-2 focus:ring-yellow-400"
              />
            </div>

            {/* Character Filter */}
            <div>
              <label htmlFor="character" className="block text-sm font-medium text-blue-100 mb-1">
                Character
              </label>
              <select
                id="character"
                ...
                className="w-full px-3 py-2 bg-[rgba(255,255,255,.1)] border border-[rgba(255,255,255,.2)] rounded-md text-white focus:outline-none focus:ring-2 focus:ring-yellow-400"
              >
            ...
            {/* Testament Filter */}
            <div>
              <label htmlFor="testament" className="block text-sm font-medium text-blue-100 mb-1">
                Testament
              </label>
              <select
                id="testament"
