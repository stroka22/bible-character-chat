...
const CharacterSelection = () => {
    ...
    return (_jsxs("div", { className: "min-h-screen bg-gradient-to-b from-[#0a0a2a] via-[#1a1a4a] to-[#2a2a6a] py-10 px-4 md:px-6", children: [renderAlphaNav(), _jsxs("div", { className: "max-w-7xl mx-auto bg-white/8 backdrop-blur-sm rounded-2xl p-6 border border-white/15 shadow-xl", children: [_jsx("h1", { className: "text-4xl md:text-5xl font-extrabold text-center text-yellow-400 mb-8 tracking-tight drop-shadow-lg", children: "Choose Your Biblical Guide" }), _jsx("div", { className: "bg-white/5 backdrop-blur-sm rounded-xl p-4 mb-6", children: _jsxs("div", { className: "flex flex-col md:flex-row gap-4 items-center", children: [_jsx("div", { className: "w-full md:flex-1", children: _jsx("input", { type: "text", placeholder: "Search characters...", value: searchQuery, onChange: (e) => {
                  ...
                  `, onClick: () => handleSelectCharacter(character.id), children: [_jsx("img", { src: character.avatar_url || `https://ui-avatars.com/api/?name=${encodeURIComponent(character.name)}&background=random`, alt: character.name, className: `
                      h-16 w-16 rounded-full object-cover border-2
                      ${selectedCharacter?.id === character.id ? 'border-yellow-400' : 'border-white/30'}
                    ` }), _jsxs("div", { className: "flex-1", children: [_jsx("h3", { className: "text-xl font-bold text-yellow-400", children: character.name }), _jsx("p", { className: "text-white/80 line-clamp-2", children: character.description }), _jsxs("div", { className: "text-xs text-white/50 mt-1", children: [character.bible_book && `${character.bible_book} • `, testament === 'old' ? 'Old Testament' : testament === 'new' ? 'New Testament' : ''] })] }), _jsx("button", { className: `
