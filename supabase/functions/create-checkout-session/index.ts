...
serve(async (req: Request) => {
  ...
  try {
    ...
    if (STRIPE_MODE === 'live' && isTestPrice) {
      console.error('Environment mismatch: Test price ID used in live mode');
    ...
    // Prepare line items with proper type checking
    const lineItems = [{
      price: priceId,
      quantity: 1,
    }];

    // Prepare metadata with proper type checking
    ...
    // Create session with properly typed parameters
    console.log('Calling stripe.checkout.sessions.create with parameters:', {
